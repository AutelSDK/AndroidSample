...
public class AutelSegmentedGroup extends RadioGroup {
    ...
    public AutelSegmentedGroup(Context context) {
        super(context);
    ...
    private void initAttrs(AttributeSet attrs) {
        TypedArray typedArray = getContext().getTheme().obtainStyledAttributes(attrs,R.styleable.AutelSegmentedGroup,0, 0);

        try {
            mMarginDp = (int) typedArray.getDimension(R.styleable.AutelSegmentedGroup_sc_border_width,getResources().getDimension(R.dimen.radio_button_stroke_border));
            mCornerRadius = typedArray.getDimension(R.styleable.AutelSegmentedGroup_sc_corner_radius,getResources().getDimension(R.dimen.radio_button_conner_radius));
            mTintColor = typedArray.getColor(R.styleable.AutelSegmentedGroup_sc_tint_color,getResources().getColor(R.color.radio_button_selected_color));
            mCheckedTextColor = typedArray.getColor(R.styleable.AutelSegmentedGroup_sc_checked_text_color,getResources().getColor(android.R.color.white));
        }
        finally {
            typedArray.recycle();
        }
    ...
    public AutelSegmentedGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        resources = getResources();
        mTintColor = resources.getColor(R.color.radio_button_selected_color);
        mMarginDp = (int) getResources().getDimension(R.dimen.radio_button_stroke_border);
...
//        }
    ...
    private class LayoutSelector {
        ...
        private int child;
        private final int SELECTED_LAYOUT = R.drawable.shape_radio_checked;
        private final int UNSELECTED_LAYOUT = R.drawable.shape_radio_unchecked;

        private final float r;    //this is the radios read by attributes or xml dimens
        ...
        private int getChildren() {
            return AutelSegmentedGroup.this.getChildCount();
        }

        private int getChildIndex(View view) {
            return AutelSegmentedGroup.this.indexOfChild(view);
        }
